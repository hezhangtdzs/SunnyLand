...
namespace engine::core {
    ...
    public:
        ...
        // 窗口设置
        std::string window_title_ = "SunnyLand";    ///< @brief 窗口标题
        int window_width_ = 1280;                   ///< @brief 窗口宽度
        int window_height_ = 720;                   ///< @brief 窗口高度
        bool window_resizable_ = true;              ///< @brief 窗口是否可调整大小
        ...
        // 音频设置
        float music_volume_ = 0.5f;             ///< @brief 音乐音量 (0.0 - 1.0)
        float sound_volume_ = 0.5f;             ///< @brief 音效音量 (0.0 - 1.0)

        // 存储动作名称到 SDL Scancode 名称列表的映射
        std::unordered_map<std::string, std::vector<std::string>> input_mappings_ = {   ///< @brief 输入映射表
